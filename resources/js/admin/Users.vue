        ...
        <div class="">
            <v-container class="pa-0">
                <v-row no-gutters>
                    ...
                    <v-col cols="3"><v-text-field v-model="searchValue" label="Search.." clearable variant="outlined"
                            class='mt-4'></v-text-field></v-col>
                    <v-col cols="3" align="right" offset="6"><v-btn color="primary" @click="addNew"><v-icon class="mr-2">fa
                                fa-plus</v-icon>Add New</v-btn></v-col>
                </v-row>
                <v-row></v-row>
            </v-container>
        </div>
                ...
                <v-card-text class="pa-5">
                    ...
                    <v-form ref="form" @submit.prevent="saveForm" lazy-validation>
                        ...
                        <v-checkbox-btn label="Update Password" v-model="enabled" class="pe-2"
                            v-if="user.id > 0"></v-checkbox-btn>

                        <template v-if="enabled">
                            <v-text-field label="Password" prepend-icon="mdi-lock" v-model="user.password"
                                variant="outlined" :rules="[rules.required]" type="password" required></v-text-field>
                            <v-text-field label="Confirm Password" prepend-icon="mdi-lock" variant="outlined"
                                v-model="user.password_confirmation"
                                :rules="[(user.password === user.password_confirmation) || 'Password must match']"
